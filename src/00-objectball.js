...
function teamColors(teamName){
  ...
  if (teamName === gameObject().home.teamName){
    colors.push(gameObject().home.colors)
  } if (teamName === gameObject().away.teamName){
    colors.push(gameObject().away.colors)
  }
  return colors
}
...
function teamName(){
  ...
  for (let teamKey in gameObject()){
    ballname.push(gameObject()[teamKey]["teamName"])
  }
  return ballname
}
  ...
  for (let teamKey in myGameObject){
    if(teamName == myGameObject[teamKey]["teamName"]){
      ...
      for (let playerKey in playersObj){
        jerseynum.push(playersObj[playerKey]["number"]);
      }
    }
  }
  return jerseynum;
...
function bigShoeRebounds(){
  const largestShoeSize = 0;
      if (players().shoe > largestShoeSize){
        largestShoePlayer = players().playerName;
      } 
    return largestShoePlayer.rebounds
  }
console.log("playerStats",bigShoeRebounds())